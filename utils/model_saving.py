"""
Checkpoint and metric-dump paths.

``save_info`` owns the directory layout under
``assets/learned_models/<algo>/<date>-exp-<algo>-<env>/`` and writes the policy /
value / cost nets, the running state normaliser, and the pickled metric lists.
"""
...
class save_info(object):
    ...
    def create_all_paths(self):
        ...
        self.model_saving_path_last = os.path.join(self.assets_dir, self.saving_path, 'model_last.p')
    ...
    def save_models_last(self, policy_net, value_net, cost_net, running_state, args):
        pickle.dump((policy_net, value_net, cost_net, running_state, args), open(os.path.join(self.assets_dir, self.model_saving_path_last), 'wb'))
