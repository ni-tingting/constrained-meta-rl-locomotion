...
``collect_samples`` runs a policy in an environment until ``min_batch_size``
transitions are gathered, returning a ``Memory`` buffer plus a log dict.
``merge_log`` combines the per-worker logs produced when sampling with several
threads. Used by the generic (non task-conditioned) code paths; the meta
