"""
Constrained MAML baseline.

Same inner/outer structure as MAML, with the cost constraint added to the inner
task update. Serves as the ablation against ``SafeMeta``'s dual-method step.
"""
...
class MAML_constraint:
    ...
    def task_specific_update(self, batch, batch1, policy=None):
        ...
        optim11 = torch.optim.SGD(this_local_policy.parameters(), lr=0.1)

        comprehesive_advantages=(reward_advantages-u*cost_advantages)/(reward_advantages-u*cost_advantages).std()
        ...
        aaaa=torch.exp(log_probs - fixed_log_probs)
        action_loss = (-comprehesive_advantages * torch.special.expit(2.0*aaaa-2.0)*2 ).mean()
        optim11.zero_grad()
    ...
    def meta_update(self, batch_list, batch_list1):
        advantages_list=[]
        costs_list=[]
        states_list=[]
        action_list=[]
        ...
        def get_loss(volatile=False):
            ...
            for states, actions, reward_advantages,fixed_log_probs11 in zip(states_list, action_list, advantages_list,fixed_log_probs_list):
                ...
                action_loss = -(reward_advantages-u * cost_advantages) * torch.exp(log_probs - fixed_log_probs11)
    ...
    def train_MAML_constraint(self, writer, save_info_obj):
        ...
        for i_iter in range(self.args.update_iter_num, self.args.max_iter_num):
            ...
            for local_iter in range(self.args.env_num):
                ...
                bbbbbbbb_average+=bbbbbbbb/self.args.env_num
            ...
            self.meta_update(batch_list, batch_list1)
            print("----------------------")
