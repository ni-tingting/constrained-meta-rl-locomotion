"""
Constrained Policy Optimisation (Achiam et al., 2017).

Single-task safe RL baseline: a trust-region policy step whose feasible set is
restricted by the discounted cost constraint, solved as a small convex program
(``cpo_problem``) with a recovery step when the constraint is violated.
"""
...
def collect_samples(pid, queue, env, env_name, env_parameter, policy,
...
class CPO:
    ...
    def collect_samples(self, env, seed, env_parameter=0.0):
        t_start = time.time()
    ...
    def meta_test(self, writer,save_info_obj):
        ...
        for m_iter in range(self.args.meta_iter_num):
            sample_time = 0
            env_parameter = env_parameter_list[m_iter % len(env_parameter_list)]
            seed = deterministic_rollout_seed(self.args.seed, m_iter % len(env_parameter_list), m_iter, 0)
            batch, log = self.collect_samples(self.envs[-1], seed, env_parameter)
            ...
            eval_seed = deterministic_rollout_seed(self.args.seed, m_iter % len(env_parameter_list), m_iter, 1)
            ...
            cvs_file=os.path.join(assets_dir(), save_info_obj.saving_path, 'test_log.csv')

            with open(cvs_file, 'a+') as file:
                writer_csv = csv.writer(file)
                writer_csv.writerow([ m_iter, sample_time, t2-t1, np.average(meta_avg_cost), eval_cost, log['env_avg_reward'], eval_reward])
    ...
    def train_CPO(self, writer, save_info_obj):
        ...
        for i_iter in range(self.args.update_iter_num, self.args.max_iter_num):
            ...
            env_parameter = create_env_parameter_list(self.args)[0]

            #self.mean_action = True
            batch, log = self.collect_samples(self.envs[0], seed, env_parameter)
            ...
            rewards_std.append(log.get('std_reward', 0.0))
            env_avg_reward.append(log.get('env_avg_reward', 0.0))
            num_of_steps.append(log.get('num_steps', 0))
            num_of_episodes.append(log.get('num_episodes', 0))
            tne = tne + log.get('num_episodes', 0)
            tns = tns + log.get('num_steps', 0)
            ...
            eval_batch, eval_log = self.collect_samples(self.envs[0], seed, env_parameter)
            ...
            # update eval lists
            eval_avg_reward.append(eval_log.get('env_avg_reward', 0.0))
            eval_avg_reward_std.append(eval_log.get('std_reward', 0.0))
            ...
            eval_env_avg_reward = eval_log.get('env_avg_reward', 0.0)
            eval_std_reward = eval_log.get('std_reward', 0.0)
            writer.add_scalar('rewards', eval_env_avg_reward, i_iter)  
            writer.add_scalar('costs', eval_cost, i_iter)  
            writer.add_scalar('std_reward', eval_std_reward, i_iter)
            ...
            if i_iter % self.args.log_interval == 0:
                print('{} T_sample {:.4f}  T_update {:.4f} C_avg/iter {:.2f}  Test_C_avg {:.2f} R_avg {:.2f} Test_R_avg {:.2f} Test_R_std {:.2f}'.format( 
                    i_iter, log['sample_time'], t1-t0, np.average(eval_avg_cost), eval_cost, log.get('env_avg_reward', 0.0), eval_env_avg_reward, eval_std_reward))
            
            # save the best model
            if eval_env_avg_reward >= best_avg_reward and eval_std_reward <= best_std:
                ...
                best_avg_reward = eval_env_avg_reward
                best_std = eval_std_reward
